%Build Figure
function getHistogramInterArrivalTime(fileName, filtered)
...
legend('inter arrival time');

%set x, y Label
set(get(axes1,'XLabel'),'String','inter arrival time (seconds)','FontSize',30,'FontWeight','bold');
...
saveas(gca, strcat(pwd,'\figs\interArrivalTimeHist_filtered', '.eps'),'psc2') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\interArrivalTimeHist_filtered'), 'pdf') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\interArrivalTimeHist_filtered'), 'png') %Save figure 
