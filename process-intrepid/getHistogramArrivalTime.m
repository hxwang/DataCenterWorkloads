%Build Figure
function getHistogramArrivalTime(fileName)
...
%set x, y Label
set(get(axes1,'XLabel'),'String','arrival time (seconds)','FontSize',30,'FontWeight','bold');
...
saveas(gca, strcat(pwd,'\figs\arrivalTimeHist', '.eps'),'psc2') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\arrivalTimeHist'), 'pdf') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\arrivalTimeHist'), 'png') %Save figure 
