...
function getCDFProcessTime(filtered)
...
data = textread('..\data\ANL-Intrepid-2009-1.swf\processTime.txt')./3600;
...
set(get(axes1,'YLabel'),'String','probability','FontSize',30,'FontWeight','bold');
...
if(filtered)
saveas(gca, strcat(pwd,'\figs\processingTimeCDF-filtered', '.eps'),'psc2') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\processingTimeCDF-filtered'), 'pdf') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\processingTimeCDF-filtered'), 'png') %Save figure     
else

saveas(gca, strcat(pwd,'\figs\processingTimeCDF', '.eps'),'psc2') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\processingTimeCDF'), 'pdf') %Save figure 
saveas(gcf, strcat(pwd ,'\figs\processingTimeCDF'), 'png') %Save figure 
end
